  ...
  // Loop method adjusts the height of bar and redraws if neccessary
	var loop = function () {
	  ...
	  for (var i = 0; i < endArr.length; i += 1) {
		...
		delta = ((endArr[i] + 1) - startArr[i]) / that.animationSteps;
		that.curArr[i] += delta;
  ...
  // Draw method updates the canvas with the current display
	var draw = function (arr) {
	  ...
	  for (i = 0; i < arr.length; i += 1) {
		...
		try {
            if (that.simbol.length === 1) {
                ctx.fillText(parseInt(arr[i],10) + that.simbol[0],
                i * that.width / numOfBars + (that.width / numOfBars) / 2,
                graphAreaHeight - barHeight - 10);
            } else {
                ctx.fillText(parseInt(arr[i],10) + that.simbol[i],
                i * that.width / numOfBars + (that.width / numOfBars) / 2,
                graphAreaHeight - barHeight - 10);
            }
		} catch (ex) {}
